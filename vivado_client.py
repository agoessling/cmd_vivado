...
class VivadoClient:
  ...
  def _command(timeout=None):
    ...
    def _decorate(function):
      ...
      def wrapped_function(self, *args, **kwargs):
        ...
        self._get_response(timeout=timeout)
  ...
  def _handle_line(self, line):
    ...
    match = re.match(b'(\w+): \[(.+)\]', line)
    if match:
      if match.group(2) in self.WHITE_LIST:
        line_type = 'INFO'
      elif match.group(1) in (b'INFO', b'WARNING', b'ERROR'):
        line_type = match.group(1).decode()
      else:
        line_type = 'INFO'

    is_error = line_type in ('WARNING', 'ERROR')
  ...
  @_command()
  def read_verilog(self, files):
    cmd = 'read_verilog {{{:s}}}\r'.format(' '.join(files))
    self.socket.sendall(cmd.encode())
...
def synthesize(client, args):
  ...
  client.read_verilog(args.verilog)
